...
const update = ({acceleration, velocity, position, mass}, delta, canvas) => {
    ...
    if ((position[0] > canvas.width) || (position[0] < 0)) {
    	velocity[0] = -1 * velocity[0]
    }
    if ((position[1] > canvas.height) || (position[1] < 0)) {
    	velocity[1] = -1 * velocity[1]
    }
    if(position[0]>canvas.width || position[0]<0) position[0] = 400;
	if(position[1]>canvas.height || position[1]<0) position[1] = 400;
    return ({ acceleration, velocity, position, mass })
}
